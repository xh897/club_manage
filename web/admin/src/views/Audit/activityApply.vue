<template>
  <div class="amazing-wrap" style="width: 90%">
    <el-alert title="活动申请列表" />
    <el-button
      style="margin: 10px"
      size="mini"
      type="primary"
      ...
      <div v-show="table" class="table">
        ...
        <el-table-column type="selection" width="40px" align="center">
          ...
          <el-table-column
            ...
            width="100px"
          />
          <el-table-column label="用户头像 (点击放大)" align="center">
  ...
  methods: {
    exportExcel () {
      if (!this.multipleSelection.length) {
        return this.$alert("请选择具体一个活动申请进行导出数据");
      }
    ...
    async fetch() {
      this.loading = true
      const data = await this.$http.getOrgActivity({
        page: this.currentPage,
        limit: this.pageSize,
        ...this.formInline,
        oid: this.$store.state.user.organizationManager
      })
    ...
    handleDelete(id) {
      this.$confirm("是否确定删除该参加活动申请", "温馨提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(async () => {
          await this.$http.deleteOrgActivity({ activity_id:id });
    ...
    handleAudit(type, value) {
      this.$confirm("是否继续操作", "温馨提示", {
        ...
        .then(async () => {
          await this.$http.updateActivityStatus({
            ...value,
            status: type,
          });

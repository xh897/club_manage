<template>
  ...
  export default {
    ...
    methods: {
      ...
      async fetch() {
        this.loading = true;
        const data = await this.$http.gatQuitAudit({
      ...
      handleDelete(id) {
        this.$confirm("是否确定删除该退出社团申请", "温馨提示", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning",
        })
          .then(async () => {
            await this.$http.deleteUserquit_association_id({ quit_association_id:id });
      ...
      handleAudit(type, value) {
        this.$confirm("是否继续操作", "温馨提示", {
          ...
          .then(async () => {
            await this.$http.setQuitAuditById({
              ...value,
              status: type,
              quitId: value.id,
            });

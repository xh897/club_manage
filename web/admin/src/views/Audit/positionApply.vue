<template>
  ...
  export default {
    ...
    methods: {
      ...
      async fetch() {
        this.loading = true;
        const data = await this.$http.gatPositionAudit({
      ...
      handleDelete(id) {
        this.$confirm("是否确定删除该职位申请", "温馨提示", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning",
        })
          .then(async () => {
            await this.$http.deletePositionAudit({ member_id:id });
      ...
      handleAudit(type, value) {
        this.$confirm("是否继续操作", "温馨提示", {
          ...
          .then(async () => {
            await this.$http.setPositionAuditById({
              ...value,
              status: type,
            });
